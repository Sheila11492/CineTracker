...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_SEGUIMIENTO + " (" +
                ...
                "fecha_inicio INTEGER, " +        // ms
                "fecha_fin INTEGER, " +           // ms
    ...
    //Crea o actualiza seguimiento con valores que le pases.
    ...
    //Marca inicio (si no existe, lo crea). Estado se pone 'Viendo' si venía sin estado.
    ...
    //Marca fin inmediatamente (estado = 'Visto' y fecha_fin = now).
    ...
    public int minutosTotalesVistos(int usuarioId) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery(
                "SELECT IFNULL(SUM(sp.duracion), 0) " +
                        "FROM " + TABLE_SERIES_PELICULAS + " sp " +
                        "JOIN " + TABLE_SEGUIMIENTO + " s ON sp.id = s.serie_pelicula_id " +
                        "WHERE s.usuario_id=? AND s.estado='Visto'",
                new String[]{String.valueOf(usuarioId)});
    ...
    private void clearRecomendacionesUsuario(SQLiteDatabase db, int usuarioId) {
        db.delete(TABLE_RECOMENDACIONES, "usuario_id=?",
                new String[]{String.valueOf(usuarioId)});
    }
    ...
    // Lee recomendaciones del usuario (cierra solo el cursor, NO la BD).
    public List<Contenido> getRecomendacionesContenido(int usuarioId) {
        ...
        try {
            ...
            if (c != null) c.close();
            // NO cerrar db (lo gestiona SQLiteOpenHelper)
        }
        return lista;
    }
    ...
    public void clearRecomendacionesUsuario(int usuarioId) {
        ...
        try {
            ...
            if (db.inTransaction()) db.endTransaction();
            // No cerramos la BD para no interferir con SQLiteOpenHelper
        }
    }
    ...
    public void insertRecomendacion(int usuarioId, int contenidoId, String criterio) {
        ...
        try {
            ...
            if (db.inTransaction()) db.endTransaction();
            // No cerramos la BD: la gestiona SQLiteOpenHelper
        }
    }
